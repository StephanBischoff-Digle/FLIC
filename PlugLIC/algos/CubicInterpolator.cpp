...
std::vector<double> CubicInterpolator::map_ts(const std::vector<double> &dss, const CubicInterpolator::DistT &dst) const {
...
std::vector<fantom::Vector2> CubicInterpolator::interpolate(const std::vector<DiffPoint> &line,
    ...
    CubicInterpolator::DistT dst{ts, ds};
    ...
    std::transform(normalized_r.begin(), normalized_r.end(), std::back_inserter(ret), [&](Vector2d v){return fantom::Vector2 {v[0], v[1]};});
